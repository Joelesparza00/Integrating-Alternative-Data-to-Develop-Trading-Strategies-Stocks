...
library(tidyverse)
library(caret)
#dowjones <- read.csv('daily_dowjones_regression.csv')
#head(dowjones)
#str(dowjones)
```

```{r}
dowjones <- read.csv('standardized_daily_dowjones_regression.csv')
head(dowjones)
str(dowjones)
```
...
x <- model.matrix(tomorrow_return ~ ., dowjones_clean)[,-1]

# Outcome
y <- dowjones_clean$tomorrow_return
...
plot_data <- data.frame(
  actual = dowjones2$tomorrow_return,
...
lower_bound <- quantile(train_dowjones$tomorrow_return, 0.025)  # Adjust percentile as needed
upper_bound <- quantile(train_dowjones$tomorrow_return, 0.975)  # Adjust percentile as needed

# Trim outliers from the training set
train_dowjones <- train_dowjones[train_dowjones$tomorrow_return >= lower_bound & train_dowjones$tomorrow_return <= upper_bound, ]
...
rt <- rpart(tomorrow_return ~ ., data = train_dowjones, method = "anova", cp = 0.0001, maxdepth = 30)
...
```{r}
mse_rt <- mean((test_dowjones$tomorrow_return - rt_pred)^2)
mse_rt
```
...
library(ggplot2)
ggplot(data = test_dowjones, aes(x = tomorrow_return, y = rt_pred)) +
...
actual_class <- ifelse(test_dowjones$tomorrow_return > 0, "Positive", "Negative")
...
dowjones_clean$binary_return <- ifelse(dowjones_clean$tomorrow_return >= 0, "Positive", "Negative")
...
category_counts <- table(ifelse(dowjones2$log_pred == 'Positive' & dowjones2$tomorrow_return > 0, "Both Positive",
                           ifelse(dowjones2$log_pred == 'Negative' & dowjones2$tomorrow_return < 0, "Both Negative",
                                  ifelse(dowjones2$log_pred == 'Positive' & dowjones2$tomorrow_return < 0, "One Positive and One Negative",
                                         ifelse(dowjones2$log_pred == 'Negative' & dowjones2$tomorrow_return > 0, "One Negative and One Positive", "Other")))))
...
write.csv(dowjones2, "Daily dowjones Trading Strategy.csv", row.names = FALSE)
```
