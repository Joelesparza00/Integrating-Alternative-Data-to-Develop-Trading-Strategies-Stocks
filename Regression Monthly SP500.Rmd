...
library(tidyverse)
library(caret)
#sp500 <- read.csv('monthly_sp500_regression.csv')
#head(sp500)
#str(sp500)
```

```{r}
sp500 <- read.csv('standardized_monthly_sp500_regression.csv')
head(sp500)
str(sp500)
```
...
x <- model.matrix(next_month_return ~ ., sp500_clean)[,-1]

# Outcome
y <- sp500_clean$next_month_return
...
plot_data <- data.frame(
  actual = sp5002$next_month_return,
...
lower_bound <- quantile(train_sp500$next_month_return, 0.025)  # Adjust percentile as needed
upper_bound <- quantile(train_sp500$next_month_return, 0.975)  # Adjust percentile as needed

# Trim outliers from the training set
train_sp500 <- train_sp500[train_sp500$next_month_return >= lower_bound & train_sp500$next_month_return <= upper_bound, ]
...
rt <- rpart(next_month_return ~ ., data = train_sp500, method = "anova", cp = 0.0001, maxdepth = 30)
...
```{r}
mse_rt <- mean((test_sp500$next_month_return - rt_pred)^2)
mse_rt
```
...
library(ggplot2)
ggplot(data = test_sp500, aes(x = next_month_return, y = rt_pred)) +
...
actual_class <- ifelse(test_sp500$next_month_return > 0, "Positive", "Negative")
...
sp500_clean$binary_return <- ifelse(sp500_clean$next_month_return >= 0, "Positive", "Negative")
...
category_counts <- table(ifelse(sp5002$log_pred == 'Positive' & sp5002$next_month_return > 0, "Both Positive",
                           ifelse(sp5002$log_pred == 'Negative' & sp5002$next_month_return < 0, "Both Negative",
                                  ifelse(sp5002$log_pred == 'Positive' & sp5002$next_month_return < 0, "One Positive and One Negative",
                                         ifelse(sp5002$log_pred == 'Negative' & sp5002$next_month_return > 0, "One Negative and One Positive", "Other")))))
...
write.csv(sp5002, "monthly sp500 Trading Strategy v2.csv", row.names = FALSE)
```
