...
library(tidyverse)
library(caret)
#russell <- read.csv('daily_russell_regression.csv')
#head(russell)
#str(russell)
```

```{r}
russell <- read.csv('standardized_daily_russell_regression.csv')
head(russell)
str(russell)
```
...
x <- model.matrix(tomorrow_return ~ ., russell_clean)[,-1]

# Outcome
y <- russell_clean$tomorrow_return
...
plot_data <- data.frame(
  actual = russell2$tomorrow_return,
...
lower_bound <- quantile(train_russell$tomorrow_return, 0.025)  # Adjust percentile as needed
upper_bound <- quantile(train_russell$tomorrow_return, 0.975)  # Adjust percentile as needed

# Trim outliers from the training set
train_russell <- train_russell[train_russell$tomorrow_return >= lower_bound & train_russell$tomorrow_return <= upper_bound, ]
...
rt <- rpart(tomorrow_return ~ ., data = train_russell, method = "anova", cp = 0.001, maxdepth = 20)
...
```{r}
mse_rt <- mean((test_russell$tomorrow_return - rt_pred)^2)
mse_rt
```
...
library(ggplot2)
ggplot(data = test_russell, aes(x = tomorrow_return, y = rt_pred)) +
...
actual_class <- ifelse(test_russell$tomorrow_return > 0, "Positive", "Negative")
...
russell_clean$binary_return <- ifelse(russell_clean$tomorrow_return >= 0, "Positive", "Negative")
...
category_counts <- table(ifelse(russell2$log_pred == 'Positive' & russell2$tomorrow_return > 0, "Both Positive",
                           ifelse(russell2$log_pred == 'Negative' & russell2$tomorrow_return < 0, "Both Negative",
                                  ifelse(russell2$log_pred == 'Positive' & russell2$tomorrow_return < 0, "One Positive and One Negative",
                                         ifelse(russell2$log_pred == 'Negative' & russell2$tomorrow_return > 0, "One Negative and One Positive", "Other")))))
...
write.csv(russell2, "Daily russell Trading Strategy.csv", row.names = FALSE)
```
