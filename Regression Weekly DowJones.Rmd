...
library(tidyverse)
library(caret)
#dowjones <- read.csv('weekly_dowjones_regression.csv')
#head(dowjones)
#str(dowjones)
```

```{r}
dowjones <- read.csv('standardized_weekly_dowjones_regression.csv')
head(dowjones)
str(dowjones)
```
...
x <- model.matrix(next_week_return ~ ., dowjones_clean)[,-1]

# Outcome
y <- dowjones_clean$next_week_return
...
plot_data <- data.frame(
  actual = dowjones2$next_week_return,
...
lower_bound <- quantile(train_dowjones$next_week_return, 0.025)  # Adjust percentile as needed
upper_bound <- quantile(train_dowjones$next_week_return, 0.975)  # Adjust percentile as needed

# Trim outliers from the training set
train_dowjones <- train_dowjones[train_dowjones$next_week_return >= lower_bound & train_dowjones$next_week_return <= upper_bound, ]
...
rt <- rpart(next_week_return ~ ., data = train_dowjones, method = "anova", cp = 0.0001, maxdepth = 30)
...
```{r}
mse_rt <- mean((test_dowjones$next_week_return - rt_pred)^2)
mse_rt
```
...
library(ggplot2)
ggplot(data = test_dowjones, aes(x = next_week_return, y = rt_pred)) +
...
actual_class <- ifelse(test_dowjones$next_week_return > 0, "Positive", "Negative")
...
dowjones_clean$binary_return <- ifelse(dowjones_clean$next_week_return >= 0, "Positive", "Negative")
...
category_counts <- table(ifelse(dowjones2$log_pred == 'Positive' & dowjones2$next_week_return > 0, "Both Positive",
                           ifelse(dowjones2$log_pred == 'Negative' & dowjones2$next_week_return < 0, "Both Negative",
                                  ifelse(dowjones2$log_pred == 'Positive' & dowjones2$next_week_return < 0, "One Positive and One Negative",
                                         ifelse(dowjones2$log_pred == 'Negative' & dowjones2$next_week_return > 0, "One Negative and One Positive", "Other")))))
...
write.csv(dowjones2, "weekly dowjones Trading Strategy v2.csv", row.names = FALSE)
```
