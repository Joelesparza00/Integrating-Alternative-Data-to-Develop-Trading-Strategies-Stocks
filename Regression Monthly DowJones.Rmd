...
library(tidyverse)
library(caret)
#dowjones <- read.csv('monthly_dowjones_regression.csv')
#head(dowjones)
#str(dowjones)
```

```{r}
dowjones <- read.csv('standardized_monthly_dowjones_regression.csv')
head(dowjones)
str(dowjones)
```

```{r}
dowjones <- dowjones %>% rename(proper = `proper.`)
...
dowjones[c("comp_size", "proper", "dummy_negative", "dummy_positive", "dummy_total")] <- 
  lapply(dowjones[c("comp_size", "proper", "dummy_negative", "dummy_positive", "dummy_total")], 
...
dowjones_ <- dowjones[, !(names(dowjones) %in% columns_to_drop)]

# Drop rows with missing values
dowjones_clean <- na.omit(dowjones_)

# Convert dowjones into a matrix and create dummy variables for character variables
x <- model.matrix(next_month_return ~ ., dowjones_clean)[,-1]

# Outcome
y <- dowjones_clean$next_month_return
...
plot_data <- data.frame(
  actual = dowjones2$next_month_return,
  s1 = dowjones2$lasso_pred
)
...
n_rows <- nrow(dowjones_clean)
n_train <- round(0.7 * n_rows)
train_index <- sample(1:n_rows, n_train)
    
# Training set
train_dowjones <- dowjones_clean[train_index, ]
    
# Test set 
test_dowjones <- dowjones_clean[-train_index, ]

# Calculate lower and upper bounds for trimming outliers
lower_bound <- quantile(train_dowjones$next_month_return, 0.025)  # Adjust percentile as needed
upper_bound <- quantile(train_dowjones$next_month_return, 0.975)  # Adjust percentile as needed

# Trim outliers from the training set
train_dowjones <- train_dowjones[train_dowjones$next_month_return >= lower_bound & train_dowjones$next_month_return <= upper_bound, ]
...
rt <- rpart(next_month_return ~ ., data = train_dowjones, method = "anova", cp = 0.0001, maxdepth = 30)
...
```{r}
# Predict
rt_pred <- predict(rt, newdata = test_dowjones, type = "vector")
...
```{r}
mse_rt <- mean((test_dowjones$next_month_return - rt_pred)^2)
mse_rt
```
...
library(ggplot2)
ggplot(data = test_dowjones, aes(x = next_month_return, y = rt_pred)) +
...
actual_class <- ifelse(test_dowjones$next_month_return > 0, "Positive", "Negative")
...
dowjones2$pred_forest <- predict(rt, newdata = dowjones2, type = "vector")
...
dowjones_clean$binary_return <- ifelse(dowjones_clean$next_month_return >= 0, "Positive", "Negative")
...
train_index <- createDataPartition(dowjones_clean$binary_return, p = 0.7, list = FALSE)
train_set <- dowjones_clean[train_index, ]
test_set <- dowjones_clean[-train_index, ]
...
dowjones2$log_pred <- predict(logit_model, newdata = dowjones2, type = "raw")
...
category_counts <- table(ifelse(dowjones2$log_pred == 'Positive' & dowjones2$next_month_return > 0, "Both Positive",
                           ifelse(dowjones2$log_pred == 'Negative' & dowjones2$next_month_return < 0, "Both Negative",
                                  ifelse(dowjones2$log_pred == 'Positive' & dowjones2$next_month_return < 0, "One Positive and One Negative",
                                         ifelse(dowjones2$log_pred == 'Negative' & dowjones2$next_month_return > 0, "One Negative and One Positive", "Other")))))
...
write.csv(dowjones2, "monthly dowjones Trading Strategy v2.csv", row.names = FALSE)
```
