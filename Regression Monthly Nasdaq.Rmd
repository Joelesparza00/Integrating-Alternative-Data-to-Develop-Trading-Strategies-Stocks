...
library(tidyverse)
library(caret)
#nasdaq <- read.csv('monthly_nasdaq_regression.csv')
#head(nasdaq)
#str(nasdaq)
```

```{r}
nasdaq <- read.csv('standardized_monthly_nasdaq_regression.csv')
head(nasdaq)
str(nasdaq)
```

```{r}
nasdaq <- nasdaq %>% rename(proper = `proper.`)
...
nasdaq[c("comp_size", "proper", "dummy_negative", "dummy_positive", "dummy_total")] <- 
  lapply(nasdaq[c("comp_size", "proper", "dummy_negative", "dummy_positive", "dummy_total")], 
...
nasdaq_ <- nasdaq[, !(names(nasdaq) %in% columns_to_drop)]

# Drop rows with missing values
nasdaq_clean <- na.omit(nasdaq_)

# Convert nasdaq into a matrix and create dummy variables for character variables
x <- model.matrix(next_month_return ~ ., nasdaq_clean)[,-1]

# Outcome
y <- nasdaq_clean$next_month_return
...
plot_data <- data.frame(
  actual = nasdaq2$next_month_return,
  s1 = nasdaq2$lasso_pred
)
...
n_rows <- nrow(nasdaq_clean)
n_train <- round(0.7 * n_rows)
train_index <- sample(1:n_rows, n_train)
    
# Training set
train_nasdaq <- nasdaq_clean[train_index, ]
    
# Test set 
test_nasdaq <- nasdaq_clean[-train_index, ]

# Calculate lower and upper bounds for trimming outliers
lower_bound <- quantile(train_nasdaq$next_month_return, 0.025)  # Adjust percentile as needed
upper_bound <- quantile(train_nasdaq$next_month_return, 0.975)  # Adjust percentile as needed

# Trim outliers from the training set
train_nasdaq <- train_nasdaq[train_nasdaq$next_month_return >= lower_bound & train_nasdaq$next_month_return <= upper_bound, ]
...
rt <- rpart(next_month_return ~ ., data = train_nasdaq, method = "anova", cp = 0.0001, maxdepth = 30)
...
```{r}
# Predict
rt_pred <- predict(rt, newdata = test_nasdaq, type = "vector")
...
```{r}
mse_rt <- mean((test_nasdaq$next_month_return - rt_pred)^2)
mse_rt
```
...
library(ggplot2)
ggplot(data = test_nasdaq, aes(x = next_month_return, y = rt_pred)) +
...
actual_class <- ifelse(test_nasdaq$next_month_return > 0, "Positive", "Negative")
...
nasdaq2$pred_forest <- predict(rt, newdata = nasdaq2, type = "vector")
...
nasdaq_clean$binary_return <- ifelse(nasdaq_clean$next_month_return >= 0, "Positive", "Negative")
...
train_index <- createDataPartition(nasdaq_clean$binary_return, p = 0.7, list = FALSE)
train_set <- nasdaq_clean[train_index, ]
test_set <- nasdaq_clean[-train_index, ]
...
nasdaq2$log_pred <- predict(logit_model, newdata = nasdaq2, type = "raw")
...
category_counts <- table(ifelse(nasdaq2$log_pred == 'Positive' & nasdaq2$next_month_return > 0, "Both Positive",
                           ifelse(nasdaq2$log_pred == 'Negative' & nasdaq2$next_month_return < 0, "Both Negative",
                                  ifelse(nasdaq2$log_pred == 'Positive' & nasdaq2$next_month_return < 0, "One Positive and One Negative",
                                         ifelse(nasdaq2$log_pred == 'Negative' & nasdaq2$next_month_return > 0, "One Negative and One Positive", "Other")))))
...
write.csv(nasdaq2, "monthly Nasdaq Trading Strategy v2.csv", row.names = FALSE)
```
