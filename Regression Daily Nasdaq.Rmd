...
library(tidyverse)
library(caret)
#nasdaq <- read.csv('daily_nasdaq_regression.csv')
#head(nasdaq)
#str(nasdaq)
```

```{r}
nasdaq <- read.csv('standardized_daily_nasdaq_regression.csv')
head(nasdaq)
str(nasdaq)
```
...
x <- model.matrix(tomorrow_return ~ ., nasdaq_clean)[,-1]

# Outcome
y <- nasdaq_clean$tomorrow_return
...
plot_data <- data.frame(
  actual = nasdaq2$tomorrow_return,
...
lower_bound <- quantile(train_nasdaq$tomorrow_return, 0.025)  # Adjust percentile as needed
upper_bound <- quantile(train_nasdaq$tomorrow_return, 0.975)  # Adjust percentile as needed

# Trim outliers from the training set
train_nasdaq <- train_nasdaq[train_nasdaq$tomorrow_return >= lower_bound & train_nasdaq$tomorrow_return <= upper_bound, ]
...
rt <- rpart(tomorrow_return ~ ., data = train_nasdaq, method = "anova", cp = 0.0001, maxdepth = 30)
...
```{r}
mse_rt <- mean((test_nasdaq$tomorrow_return - rt_pred)^2)
mse_rt
```
...
library(ggplot2)
ggplot(data = test_nasdaq, aes(x = tomorrow_return, y = rt_pred)) +
...
actual_class <- ifelse(test_nasdaq$tomorrow_return > 0, "Positive", "Negative")
...
nasdaq_clean$binary_return <- ifelse(nasdaq_clean$tomorrow_return >= 0, "Positive", "Negative")
...
category_counts <- table(ifelse(nasdaq2$log_pred == 'Positive' & nasdaq2$tomorrow_return > 0, "Both Positive",
                           ifelse(nasdaq2$log_pred == 'Negative' & nasdaq2$tomorrow_return < 0, "Both Negative",
                                  ifelse(nasdaq2$log_pred == 'Positive' & nasdaq2$tomorrow_return < 0, "One Positive and One Negative",
                                         ifelse(nasdaq2$log_pred == 'Negative' & nasdaq2$tomorrow_return > 0, "One Negative and One Positive", "Other")))))
...
write.csv(nasdaq2, "Daily Nasdaq Trading Strategy v2.csv", row.names = FALSE)
```
